...
1. 安装：把仓库克隆到对应 Agent 的 skills 目录，或使用 `scripts/install.sh <target>`。
2. 调用：让 Agent 使用 `$business-website-skill`，并说明是“先出蓝图”还是“直接做原型”。
3. 验收：正式交付前运行 `audit_static_site.py --strict-seo --no-placeholders`，确保 SEO/GEO 元数据和占位符都处理完。
...
生成的 `dist/business-website-skill-runtime.zip` 可以发给团队成员解压到他们的 skills 目录。
